...
export class GeoColService {
  ...
  uploadUrbanismeDocument(form: FormData): Observable<any> {
    return this.http.post(
      "http://localhost:3000/geo-col/ubanismeDocument",
      form,
      { observe: "response" }
    );
  }
  getAllUrbanismeDocuments() {
    ...
    return this.http.get("http://localhost:3000/geo-col/urbanismeDocument", {
      headers: headers,
    });
  }
  fetchGeoJSONFiles(file: string) {
    console.log("Start fetching");
    const linkToFile = "../../../assets/urba.txt";
    return this.http.get(linkToFile);
  }
  ...
  uploadLogos(form: FormData) {
    return this.http
      .post("http://localhost:3000/geo-urba/pdfLogos", form)
  ...
  getLogosByCommune(commune:string):Observable<any[]>{
    return this.http.get(`http://localhost:3000/geo-urba/pdfLogos/${commune}`).pipe(map((data:any[])=>{
      const logosArray=[];
      if(data.length!=0){
        data.forEach(elem=>{
  ...
  convertCoords(latlngArray:any[]){
    ...
    return this.http.post("http://127.0.0.1:5000/coordsConverter",{coords:[latlngArray]})
  }
  uploadGeotiffs(form:FormData,filename:string){
    this.http.post("http://127.0.0.1:5000/uploadGeoTIFF",form,{reportProgress:true,responseType:'json',observe:'events'}).subscribe((event:any)=>{
  ...
  convertToWGS84(array:any[]){
    return this.http.post("http://127.0.0.1:5000/revertCoordsConverter",{coords:[array]}).pipe(map((data:any)=>{
      let polygonCoords=[];
  ...
  getAllProjects(){
   return  this.http.get('http://localhost:3000/geo-ins/allProjects');
  }
}
